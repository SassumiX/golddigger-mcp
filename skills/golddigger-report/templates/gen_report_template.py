#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate 普兰卡钎具 客户画像雷达报告 Word文档 (with real radar charts)"""
...
OUTPUT = "普兰卡钎具_客户画像雷达报告.docx"
CHART_DIR = "radar_charts"
...
def style_run(run, font_name='Arial', size=11, bold=False, color=None, italic=False):
...
def add_bullet(doc, text, text_color=None, size=10.5):
...
def radar_row(doc, label, score):
    bar_width = int(score / 100 * 10)
    bar = '█' * bar_width + '░' * (10 - bar_width)
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(3)
    p.paragraph_format.space_after = Pt(3)
    r1 = p.add_run(f'{label:<12}')
    style_run(r1, size=10, color=GRAY)
    r2 = p.add_run(bar)
...
# ==================== 封面 ====================
add_heading(doc, '普兰卡钎具', level=1, color=GOLD, center=True, size=30)
...
add_para(doc, '客户画像雷达报告', color=DARK, bold=True, center=True, size=28)
...
# 综合对比雷达图
doc.add_paragraph()
add_heading(doc, '综合对比雷达图', level=2, color=DARK)
add_hr(doc)
add_image(doc, f"{CHART_DIR}/comparison_radar.png", width=Cm(14))
add_para(doc, 'Afrimat (红)  ·  White Rock (蓝)  ·  Bin Harkil (橙)', color=GRAY,
         center=True, size=9, space_before=2, space_after=8)
...
add_table(doc, headers, rows, col_widths=[3.2, 4.2, 4.2, 4.2])
doc.add_paragraph()
...
r_t1 = p_tag.add_run('🇿🇦 南非  ')
style_run(r_t1, size=11, color=GRAY)
r_t2 = p_tag.add_run('🔥 91/100  ')
style_run(r_t2, size=13, bold=True, color=HOT_RED)
r_t3 = p_tag.add_run('HOT LEAD')
style_run(r_t3, size=11, bold=True, color=HOT_RED)
...
doc.add_paragraph()
add_image(doc, f"{CHART_DIR}/afrimat_limited_radar.png", width=Cm(11))
...
r_t21 = p_tag2.add_run('🇦🇪 阿联酋  ')
style_run(r_t21, size=11, color=GRAY)
r_t22 = p_tag2.add_run('🔥 88/100  ')
style_run(r_t22, size=13, bold=True, color=WARM_ORG)
r_t23 = p_tag2.add_run('HOT LEAD')
style_run(r_t23, size=11, bold=True, color=WARM_ORG)
...
doc.add_paragraph()
add_image(doc, f"{CHART_DIR}/white_rock_minerals_radar.png", width=Cm(11))
...
doc.add_paragraph()
add_image(doc, f"{CHART_DIR}/bin_harkil_group_radar.png", width=Cm(11))
